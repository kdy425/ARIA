#include <stdio.h>
#include <stdlib.h>
#include<time.h>
typedef unsigned char Byte;

const Byte S[4][256] = {
	// S-box type 1
{
  },
  // S-box type 2
{
  },
  // inverse of S-box type 1
{
  },
  // inverse of S-box type 2
{
  }
};
// constants used in the round key generation
const Byte KRK[3][16] = {
  {0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94, 0xfe, 0x13, 0xab, 0xe8, 0xfa, 0x9a, 0x6e, 0xe0},
  {0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20, 0xff, 0x28, 0xb1, 0xd5, 0xef, 0x5d, 0xe2, 0xb0},
  {0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70, 0x03, 0x24, 0x97, 0x75, 0x04, 0xe8, 0xc9, 0x0e}
};

// 마스킹 값 생성 함수
void generate_masks(Byte* m, Byte* m_prime, Byte* m_double_prime) {
	*m = (Byte)(rand() & 0xFF);
	*m_prime = (Byte)(rand() & 0xFF);
	*m_double_prime = *m ^ *m_prime;
}

// 마스킹된 S-box를 저장할 배열
Byte MS[4][256] = { 0, };

// 마스킹된 S-box 생성 함수
void generate_masked_sbox(const Byte S[4][256], Byte m, Byte m_prime) {
	for (int x = 0; x < 256; x++) {
		Byte u = x ^ m;
		Byte v = S[0][x] ^ m_prime;
		Byte w = S[1][x] ^ m_prime;
		MS[0][u] = v;
		MS[1][u] = w;
		MS[2][v] = x ^ m;
		MS[3][w] = x ^ m;
	}
}	

//masking sbox 검증 함수
int verify_masked_sbox(Byte m, Byte m_prime) {
	for (int x = 0; x < 256; x++) {
		// MS_3(x ^ m') = S_1^(-1)(x) ^ m 확인
		if (MS[2][x ^ m_prime] != (S[2][x] ^ m)) {
			printf("MS_3 검증 실패: x = %d\n", x);
			return 0;
		}

		// MS_4(x ^ m') = S_2^(-1)(x) ^ m 확인
		if (MS[3][x ^ m_prime] != (S[3][x] ^ m)) {
			printf("MS_4 검증 실패: x = %d\n", x);
			return 0;
		}
	}
	return 1;
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


// Diffusion Layer
//확산계층
void DL(const Byte* i, Byte* o)
{
	Byte T;

	T = i[3] ^ i[4] ^ i[9] ^ i[14];
	o[0] = i[6] ^ i[8] ^ i[13] ^ T;
	o[5] = i[1] ^ i[10] ^ i[15] ^ T;
	o[11] = i[2] ^ i[7] ^ i[12] ^ T;
	o[14] = i[0] ^ i[5] ^ i[11] ^ T;
	T = i[2] ^ i[5] ^ i[8] ^ i[15];
	o[1] = i[7] ^ i[9] ^ i[12] ^ T;
	o[4] = i[0] ^ i[11] ^ i[14] ^ T;
	o[10] = i[3] ^ i[6] ^ i[13] ^ T;
	o[15] = i[1] ^ i[4] ^ i[10] ^ T;
	T = i[1] ^ i[6] ^ i[11] ^ i[12];
	o[2] = i[4] ^ i[10] ^ i[15] ^ T;
	o[7] = i[3] ^ i[8] ^ i[13] ^ T;
	o[9] = i[0] ^ i[5] ^ i[14] ^ T;
	o[12] = i[2] ^ i[7] ^ i[9] ^ T;
	T = i[0] ^ i[7] ^ i[10] ^ i[13];
	o[3] = i[5] ^ i[11] ^ i[14] ^ T;
	o[6] = i[2] ^ i[9] ^ i[12] ^ T;
	o[8] = i[1] ^ i[4] ^ i[15] ^ T;
	o[13] = i[3] ^ i[6] ^ i[8] ^ T;
}

void RotXOR(const Byte* s, int n, Byte* t)
{
	int i, q;

	q = n / 8; n %= 8;
	for (i = 0; i < 16; i++) {
		t[(q + i) % 16] ^= (s[i] >> n);
		if (n != 0) t[(q + i + 1) % 16] ^= (s[i] << (8 - n));
	}
}

void printBlock(Byte* b);

//암호화 라운드키 생성_masking
int EncKeySetup(const Byte* w0, Byte* e, int keyBits, const Byte m_double_prime) {
	int i, R = (keyBits + 256) / 32, q;
	Byte t[16], w1[16], w2[16], w3[16];

	q = (keyBits - 128) / 64;
	for (i = 0; i < 16; i++) t[i] = S[i % 4][KRK[q][i] ^ w0[i]];
	DL(t, w1);
	if (R == 14)
		for (i = 0; i < 8; i++) w1[i] ^= w0[16 + i];
	else if (R == 16)
		for (i = 0; i < 16; i++) w1[i] ^= w0[16 + i];

	q = (q == 2) ? 0 : (q + 1);
	for (i = 0; i < 16; i++) t[i] = S[(2 + i) % 4][KRK[q][i] ^ w1[i]];
	DL(t, w2);
	for (i = 0; i < 16; i++) w2[i] ^= w0[i];

	q = (q == 2) ? 0 : (q + 1);
	for (i = 0; i < 16; i++) t[i] = S[i % 4][KRK[q][i] ^ w2[i]];
	DL(t, w3);
	for (i = 0; i < 16; i++) w3[i] ^= w1[i];

	for (i = 0; i < 16 * (R + 1); i++) e[i] = 0;

	RotXOR(w0, 0, e); RotXOR(w1, 19, e);
	RotXOR(w1, 0, e + 16); RotXOR(w2, 19, e + 16);
	RotXOR(w2, 0, e + 32); RotXOR(w3, 19, e + 32);
	RotXOR(w3, 0, e + 48); RotXOR(w0, 19, e + 48);
	RotXOR(w0, 0, e + 64); RotXOR(w1, 31, e + 64);
	RotXOR(w1, 0, e + 80); RotXOR(w2, 31, e + 80);
	RotXOR(w2, 0, e + 96); RotXOR(w3, 31, e + 96);
	RotXOR(w3, 0, e + 112); RotXOR(w0, 31, e + 112);
	RotXOR(w0, 0, e + 128); RotXOR(w1, 67, e + 128);
	RotXOR(w1, 0, e + 144); RotXOR(w2, 67, e + 144);
	RotXOR(w2, 0, e + 160); RotXOR(w3, 67, e + 160);

	RotXOR(w3, 0, e + 176); RotXOR(w0, 67, e + 176);
	RotXOR(w0, 0, e + 192); RotXOR(w1, 97, e + 192);
	if (R > 12) {
		RotXOR(w1, 0, e + 208); RotXOR(w2, 97, e + 208);
		RotXOR(w2, 0, e + 224); RotXOR(w3, 97, e + 224);
	}
	if (R > 14) {
		RotXOR(w3, 0, e + 240); RotXOR(w0, 97, e + 240);
		RotXOR(w0, 0, e + 256); RotXOR(w1, 109, e + 256);
	}

	//m'' 으로 모든 라운드키 마스킹
	for (i = 0; i < 16 * (R + 1); i++) {
		e[i] ^= m_double_prime;
	}

	return R;
}
// Decryption round key generation rountine
// w0 : maskter key, d : decryption round keys
// Encryption and decryption rountine
// p: plain text, e: round keys, c: ciphertext

void Crypt(const Byte* p, int R, const Byte* e, Byte* c, Byte m, Byte m_prime, Byte m_double_prime){
	int i, j;
	Byte t[16];

	// 초기화: 입력 복사
	for (j = 0; j < 16; j++) c[j] = p[j];

	for (i = 0; i < R / 2; i++){
		// 홀수 라운드
		for (j = 0; j < 16; j++) t[j] = MS[j % 4][e[j] ^ c[j]];
		DL(t, c);

		// 확산 계층 후 마스킹 조정
		for (j = 0; j < 16; j++) {
			if ((j / 4) % 2 == 1) c[j] ^= m_double_prime;
		}
		e += 16;

		if (i == (R / 2) - 1) // 마지막 라운드는 계산하지 않음
			break;

		// 짝수 라운드
		for (j = 0; j < 16; j++) t[j] = MS[(2 + j) % 4][e[j] ^ c[j]];
		DL(t, c);

		// 확산 계층 후 마스킹 조정
		for (j = 0; j < 16; j++) {
			if ((j / 4) % 2 == 1) c[j] ^= m_double_prime;
		}
		e += 16;
	}


	//마지막 라운드
	for (j = 0; j < 16; j++) {
		t[j] = MS[(2 + j) % 4][e[j] ^ c[j]];	//m m m' m'
		c[j] = e[16 + j] ^ t[j];	//m' m' m  m
	}
}


void printBlockOfLength(Byte* b, int len) {
	int i;

	for (i = 0; i < len; i++, b++) {
		printf("%02x", *b);
		if (i % 4 == 3 && i < len - 1) printf(" ");
	}
}

void printBlock(Byte* b) {
	printBlockOfLength(b, 16);
}

void ARIA_test() {
	Byte rk[13 * 17], c[16], mk[16];
	Byte p[16] = { 0x11, 0x11, 0x11, 0x11, 0xaa, 0xaa, 0xaa, 0xaa, 0x11, 0x11, 0x11, 0x11, 0xbb, 0xbb, 0xbb, 0xbb };
	const Byte cryptResult[] = {
	  0xc6, 0xec, 0xd0, 0x8e, 0x22, 0xc3, 0x0a, 0xbd, 0xb2, 0x15, 0xcf, 0x74, 0xe2, 0x07, 0x5e, 0x6e };
	int i, flag;

	Byte m, m_prime, m_double_prime;

	// 마스킹 값 생성
	generate_masks(&m, &m_prime, &m_double_prime);

	// 마스킹된 S-box 생성
	generate_masked_sbox(S, m, m_prime);

	for (i = 0; i < 16; i++)
		mk[i] = i * 0x11;

	// 평문 마스킹
	Byte masked_p[16];
	for (i = 0; i < 16; i++) {
		masked_p[i] = p[i] ^ ((i % 4 < 2) ? m_prime : m);
	}

	// 마스킹된 암호화
	Crypt(masked_p, EncKeySetup(mk, rk, 128, m_double_prime), rk, c, m, m_prime, m_double_prime);

	// 암호문 마스킹 제거
	for (i = 0; i < 16; i++) {
		c[i] ^= ((i % 4 < 2) ? m_prime : m);
	}

	printf("BEGIN testing basic encryption...\n");
	printf("Testing whether the encryption would come out correctly, \
for 12-round ARIA.\n");
	printf("key      : "); printBlockOfLength(mk, 16); printf("\n");
	printf("plaintext: "); printBlock(p); printf("\n");
	printf("result is: "); printBlock(c); printf("\n");
	printf("should be: "); printBlock((Byte*)cryptResult); printf("\n");
	flag = 0;
	for (i = 0; i < 16; i++)
		if (c[i] != cryptResult[i])
			flag = 1;
	if (flag == 1)
		printf("The result is incorrect!\n");
	else
		printf("Okay.  The result is correct.\n");
	printf("END   testing basic encryption.\n\n");


}

int main(int argc, char** argv) {
	
	/*
	srand(time(NULL));
	Byte m, m_prime, m_double_prime;
	generate_masks(&m, &m_prime, &m_double_prime);
	generate_masked_sbox(S, m, m_prime);
	// 마스킹 S-box 검증
	if (verify_masked_sbox(m, m_prime)) {
		printf("마스킹 S-box 검증 성공\n");
	}
	else {
		printf("마스킹 S-box 검증 실패\n");
	}
	return 0;
	*/

	ARIA_test();




	
	return 0;
}

